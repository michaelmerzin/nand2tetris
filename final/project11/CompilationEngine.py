...
class CompilationEngine:
    ...
    def compile_term(self) -> None:
        ...
        if self.input_stream.token_type() == "IDENTIFIER":
            ...
            try:
                ...
                try:
                    output = ""
                    output += self.compile_subroutine_call(True, name=name, already_pushed=already_pushed)
                    self.count_pushed += 1
    ...
    def compile_subroutine_call(self, is_identifier_compiled, name="", already_pushed=False) -> str:
        tabs = self.count_tabs
        ...
        try:
            output = self.compile_class_function(output, name=name, already_pushed=already_pushed)
    ...
    def compile_class_function(self, base_output, name="", already_pushed=False) -> str:
        output = base_output
        output += self.handle_words(["."])
        ...
        if self.symbol_table.type_of(name) and not already_pushed:
            segment = self.symbol_table.kind_of(name)
            index = self.symbol_table.index_of(name)
            self.vm_writer.write_push(self.kind_to_segment(segment), index)
    ...
    def compile_function(self, base_output, func_name="") -> str:
        ...
        output = ""
        if is_method:
            self.vm_writer.write_push(VMWriter.POINTER, 0)
        self.compile_expression_list()
        output += self.handle_words([")"])

        self.count_pushed += 1
        final_name = f"{self.cur_class_name}.{func_name}"
        self.vm_writer.write_call(final_name, self.count_pushed)
        return output
